...
public class SampleBlockVanilla extends Block implements IMetaName,IHasModel {

	public static PropertyEnum<Types.EnumTypeBasic> VARIANT = PropertyEnum.<Types.EnumTypeBasic>create("variant", Types.EnumTypeBasic.class);
	public static final AxisAlignedBB SampleBlockAABB = new AxisAlignedBB(0.25D, 0, 0.25D, 0.75D, 0.125D, 0.75D);
	
	public static SampleBlockVanilla INSTANCE;
	
	public SampleBlockVanilla(String name) {
		super(Material.ROCK);
		setUnlocalizedName(name);
		setRegistryName(name);
		setCreativeTab(CreativeTabs.BUILDING_BLOCKS);
		setDefaultState(this.blockState.getBaseState().withProperty(VARIANT,  Types.EnumTypeBasic.values()[0]));
	...
	public int damageDropped(IBlockState state) {
		...
		return ((Types.EnumTypeBasic)state.getValue(VARIANT)).getMeta();
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		...
		return this.getDefaultState().withProperty(VARIANT, Types.EnumTypeBasic.byMetadata(meta));
	}
	...
	public void getSubBlocks(CreativeTabs itemIn, NonNullList<ItemStack> items) {
		...
		for(EnumTypeBasic variant: Types.EnumTypeBasic.values()) {
	...
	public void registerModels() {
		for (int i = 0; i < Types.EnumTypeBasic.values().length; i++) {
			SimpleOreSamples.proxy.registerVariantRenderer(Item.getItemFromBlock(this), i, "sample_" + EnumTypeBasic.values()[0].getName(), "inventory");
		}
	...
	public static class ColorHandler implements IBlockColor, IItemColor {
		...
		public int getColorFromItemstack(ItemStack stack, int tintIndex) {
			return (tintIndex == 0 ? -1 : Types.EnumTypeBasic.values()[stack.getItemDamage()].getColor());
		}
		...
		public int colorMultiplier(IBlockState state, IBlockAccess worldIn, BlockPos pos, int tintIndex) {
			int color = (tintIndex == 0 ? -1 :  Types.EnumTypeBasic.values()[state.getBlock().getMetaFromState(state)].getColor()  );
			return color;
		}
	}
